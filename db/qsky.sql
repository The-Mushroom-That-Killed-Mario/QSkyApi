...
CREATE INDEX `idx_branch_branch` ON `branch` (`parent_id` ASC);

CREATE UNIQUE INDEX `branch_id_UNIQUE` ON `branch` (`branch_id` ASC);
...
CREATE UNIQUE INDEX `id_UNIQUE` ON `employee` (`id` ASC);
...
CREATE INDEX `idx_step_before_step_id` ON `step` (`before_step_id` ASC);

CREATE INDEX `idx_step_after_step_id` ON `step` (`after_step_id` ASC);
...
CREATE INDEX `idx_customer_before_customer_id` ON `customer` (`before_customer_id` ASC);

CREATE INDEX `idx_customer_after_customer_id` ON `customer` (`after_customer_id` ASC);

CREATE INDEX `idx_customer_first_step_id` ON `customer` (`first_step_id` ASC);


-- -----------------------------------------------------
-- Table `pager_data`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pager_data` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pager_quiz_items` ;
...
-- Table `pager_results`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pager_results` ;
...
CREATE TABLE IF NOT EXISTS `pager_results` (
  ...
  `qsys_mac` VARCHAR(45) NULL DEFAULT '',
  `qsys_token` VARCHAR(45) NULL DEFAULT '',
  `qsys_checkdb` INT NULL DEFAULT -2,
  `qsys_usrs` INT NULL DEFAULT 0,
  `qsys_srvs` INT NULL DEFAULT 0,
  `qsys_nm` VARCHAR(1024) NULL DEFAULT '',
  PRIMARY KEY (`id`),
...
CREATE INDEX `idx_pager_results_pager_data` ON `pager_results` (`pager_data_id` ASC);

CREATE INDEX `idx_pager_results_quiz` ON `pager_results` (`quiz_id` ASC);
...
CREATE TABLE IF NOT EXISTS `auth` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `token_auth` VARCHAR(128) NOT NULL COMMENT 'Токен пользователя для идентификации',
  `endpoint_api` VARCHAR(256) NOT NULL COMMENT 'URL сервера API',
  `comments` VARCHAR(256) NULL COMMENT 'Комментарии к доступу',
  `expire` DATE NULL COMMENT 'Дата прекращения доступа',
  `start` DATE NULL COMMENT 'Дата начала доступа',
  PRIMARY KEY (`id`))
ENGINE = InnoDB;

CREATE UNIQUE INDEX `token_UNIQUE` ON `auth` (`token_auth` ASC);
